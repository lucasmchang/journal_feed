...
class NeuronParser(HTMLParser):
        def __init__(self):
            super(NeuronParser, self).__init__()
        ...
        def feed(self, data):
            super(NeuronParser, self).feed(data)
            self.quality_check()
        ...
        def quality_check(self):
            ...
            if len(self.titles) < 12:
                self.warnings += "Warning: found too few articles in Neuron"
            if sum([x == [] for x in self.authors]) > 6:
                self.warnings += "Warning: found many articles with no authors in Neuron"
            if sum([x == '' for x in self.descriptions]) > 6:
                self.warnings += "Warning: found many articles with no description in Neuron"
